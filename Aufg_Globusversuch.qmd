...
d <-
  tibble(
    p_Gitter = seq(from = 0, to = 1, by = 0.1), # <1>
    Priori  = 1) %>% # <2>
    mutate(
      Likelihood = dbinom(14, size = 20, prob = p_Gitter), # <3>
      unstd_Post = Likelihood * Priori, # <4>
      Post = unstd_Post / sum(unstd_Post)) %>% # <5>
    mutate(
      Post = round(Post, digits = 2)) # <6>
d %>%
  arrange(-Post)
```

1. Definiere das Gitter.
2. Bestimme die Priori-Werte.
3. Berechne Likelihood für jeden Gitterwert.
4. Berechne unstand. Posteriori-Werte.
5. Berechne stand. Posteriori-Werte (summiert zu 1).
6. Runde die Posteriori-Werte auf 2 Dezimalstellen (oder eine andere gewünschte Anzahl).
...
d3 <-
  tibble(
    p_Gitter = seq(from = 0, to = 1, by = 0.1), # <1>
    Priori  = case_when(p_Gitter < 0.5 ~ 0, # <2>
                        p_Gitter >= 0.5 ~ 1)) %>% # <3>
    mutate(
      Likelihood = dbinom(14, size = 20, prob = p_Gitter), # <4>
      unstd_Post = Likelihood * Priori, # <5>
      Post = unstd_Post / sum(unstd_Post)) %>% # <6>
    mutate(
      Post = round(Post, digits = 2)) # <7>
d3
```
1. Definiere das Gitter.
2. Für alle p_Gitter-Werte unter 0.5 gilt apriori 0.
3. Für alle p_Gitter-Werte über oder gleich 0.5 gilt apriori 1.
4. Berechne Likelihood für jeden Gitterwert.
5. Berechne unstand. Posteriori-Werte.
6. Berechne stand. Posteriori-Werte (summiert zu 1).
7. Runde die Posteriori-Werte auf 2 Dezimalstellen (oder eine andere gewünschte Anzahl).
:::
