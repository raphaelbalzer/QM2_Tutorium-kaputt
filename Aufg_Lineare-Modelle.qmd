...
### A1
Das folgende Modell soll die Größe der Menschen einer Population anhand des Körpergewichts
vorher sagen. Welches der folgenden Modelle ist am besten für den genannten Zweck definiert?
...
### L1
**Antwort A:**\
Die Modelldefinition der Option A ist zwar grundsätzlich nicht falsch, geht aber davon aus, dass der
Anstieg unseres linearen Modells $\beta \sim N(0,10)$ positiv sowie negativ sein könnte. Genauer gesagt, gibt die angegebene Normalverteilung an, dass die Wahrscheinlichkeit gleich groß für positive und negative Werte ist ($\mu = 0,\pm1\ SD = -10 bis +10$). Praktisch interpretiert heißt das: Das Modell nimmt an, dass Menschen, die immer schwerer werden, mit gleich großer
Wahrscheinlichkeit größer oder klein
er werden könnten. Eine bessere Modelldefinition wäre allerdings, dass Menschen, die immer schwerer werden auch mit
größerer Wahrscheinlichkeit größer werden.\
\
**Antwort B:**\
Modell B enthält einen Formfehler. Sowohl im Likelihood als auch im linearen Modell fehlen die Beobachtungen $i$. Entsprechend wird laut Modelldefinition keine konkrete Beobachtung
vorhergesagt, was ja unter anderem der Sinn eines Modells ist.\
\
**Antwort C:**\
...
vorhersagen soll, ist dieses von den aufgelisteten
am besten geeignet.\
\
**Antwort D:**\
In Modell D ist der Anstieg $\beta$ unrealistisch groß definiert. Das Modell nimmt apriori an, dass es möglich ist, dass ein Mensch mit einem Gewicht von
$x=0$ sich von einem Menschen mit $x=1$ um 140 cm unterscheidet. Praktisch gesprochen:\
Wenn wir die Körpergröße zweier Menschen vergleichen, die sich in ihrem Körpergewicht nur um einen Kilogramm unterscheiden (Mensch 1 = 60 kg, Mensch 2 = 61 kg), dann wäre es laut dem
Modell möglich, dass Mensch 2 auf Grundlage seines höheren Gewichts 140 cm größer sein könnte als Mensch 1. Da das in der Realität aber (fast) nie der Fall sein wird, handelt es sich um keine plausible Annahme.
:::
...
Verwenden Sie für die folgenden Aufgaben den Datensatz "mtcars" aus dem Paket tidyverse.
...
#### A4 a)
Erstellen Sie ein Modell, dessen AV der Spritverbrauch und dessen UV das zentrierte Gewicht eines Autos ist.

#### L4 a)
```{r}
m3 <- stan_glm(mpg ~ weight_c, data = mtcars, refresh = 0)
m3
```
:::
...
#### A4 b)
Berechnen Sie die PPV für die Prädiktorwerte x = - 1, x = 0, x = 1.

#### L4 b)
```{r}
weight_df <- tibble(weight_c = c(- 1, 0, 1)) #Wertebereich von -1 bis 1 für den Prädiktor "Gewicht" aufstellen
```
...
#### A4 c)
Welcher Spritverbrauch wird von den Autos, die ein durchschnittliches Gewicht haben, mit einer Wahrscheinlichkeit von 90 % nicht überschritten?

#### L4 c)
```{r}
ppv3%>%
  filter(weight_c == 2)%>%
  summarise(quant90 = quantile(mpg, prob = .9))
```
:::
...
#### A4 d)
Wie groß ist die Wahrscheinlichkeit, dass Autos, mit einer Ausprägung x = -1, x = 0 oder x = 1, einen Spritverbrauch haben, der kleiner ist als 21 mpg? 
...
#### A4 e)
Was ist der Mittelwert und was die SD in der PPV für Autos, die eine Tonne schwerer als der Schnitt sind (x = 1)?
