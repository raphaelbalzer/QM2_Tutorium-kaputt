...
```{r, class='floating-code'}
set.seed(42)
m1a <- stan_glm(kid_score ~ mom_iq, data = kidiq, refresh = 0) # <1>
parameters(m1a)
```
...
kidiq <- 
  kidiq%>%
  mutate(mom_iq_c = mom_iq - mean(mom_iq)) # <1>
```
1. Ein Wert in der Spalte mom_iq minus den Mittelwert der Spalte mom_iq

```{r, class='floating-code'}
set.seed(42)
m1b <- stan_glm(kid_score ~ mom_iq_c, data = kidiq, refresh = 0) # <1>
parameters(m1b)
```
1. Modell mit zentriertem Prädiktor
...
m1btab <- m1b%>% # <1>
  as_tibble()
```
1. Modell m1b als Tabelle ausgeben
...
m1btab_f <-
  m1btab%>%
  mutate(x5 = `(Intercept)` + mom_iq_c*5) # <1>
```
1. Spalte mutieren, in der man zum Intercept (x = 0), fünf Einheiten von x hinzufügt
...
mtcars <-
  mtcars%>%
  mutate(weight_c = center(wt)) # <1>
```
1. Gewicht zentrieren
