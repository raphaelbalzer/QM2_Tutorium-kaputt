...
normalv%>%
  filter(Verteilung >= X)%>%
  summarise(Anteil = 100*(n() / 100000)) #um die Wahrscheinlichkeit direkt in Prozent zu bekommen
```
...
In welchem Intervall liegen 95% der Werte?

```{r, eval=FALSE}
normalv%>%
  summarise(quan5 = quantile(Verteilung, prob = .05))
normalv%>%
  summarise(quan95 = quantile(Verteilung, prob = .95))
```
:::
